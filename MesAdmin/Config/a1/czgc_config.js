{
  ...
  batoperate: {
    import_by_add: function (_this, res) {
      if (res.files.length > 0) {
        ...
        try {
          _this.$request('get', '/a1/czgc/readxls', {
            fileid: fid
    ...
    import_by_replace: function (_this, res) {
      if (res.files.length > 0) {
        var fid = res.files[0].fileid;
        try {
          _this.$request('get', '/a1/czgc/readxls_by_replace', {
            fileid: fid
          }).then(function (result) {
            _this.$loading().close();
            if (result.code === 1) {
    ...
    import_by_zh: function (_this, res) {
      if (res.files.length > 0) {
        var fid = res.files[0].fileid;
        try {
          _this.$request('get', '/a1/czgc/readxls_by_zh', {
            fileid: fid
          }).then(function (result) {
            _this.$loading().close();
            if (result.code === 1) {
  ...
  pagefuns: {
    ...
    download_template_file: function () {
      window.open('http://172.16.201.216:7002/template/A1/操作规程.xlsx?r=' + Math.random());
    },
    view_dzgypdf: function (row) {
      window.open("http://172.16.201.216:81/操作规程/" + row.wjlj + '/' + row.gymc);
    },
