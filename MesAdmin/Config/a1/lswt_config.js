{
  ...
  batoperate: {
    import_by_add: function (_this, res) {
      if (res.files.length > 0) {
        ...
        try {
          _this.$request('get', '/a1/lswt/readxls', {
            fileid: fid
    ...
    import_by_replace: function (_this, res) {
      if (res.files.length > 0) {
        var fid = res.files[0].fileid;
        try {
          _this.$request('get', '/a1/lswt/readxls_by_replace', {
            fileid: fid
          }).then(function (result) {
            _this.$loading().close();
            if (result.code === 1) {
    ...
    import_by_zh: function (_this, res) {
      if (res.files.length > 0) {
        var fid = res.files[0].fileid;
        try {
          _this.$request('get', '/a1/lswt/readxls_by_zh', {
            fileid: fid
          }).then(function (result) {
            _this.$loading().close();
            if (result.code === 1) {
  ...
  pagefuns: {
    ...
    download_template_file: function () {
      window.open('http://172.16.201.216:7002/template/A1/历史问题.xlsx?r=' + Math.random());
    },
    view_dzgypdf: function (row) {
      window.open("http://172.16.201.216:81/历史问题/" + row.wjlj + '/' + row.gymc);
    },
