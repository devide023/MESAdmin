{
  ...
  operate_fnlist: [{
      label: '上传',
      btntype: 'upload',
      action: 'http://192.168.1.111:7002/api/ducar/upload/dzgy_pdf',
  ...
  batoperate: {
    import_by_add: function (_this, res) {
      if (res.files.length > 0) {
        ...
        try {
          _this.$request('get', '/ducar/dzgy/readxls', {
            fileid: fid
    ...
    import_by_replace(_this, res) {
      if (res.files.length > 0) {
        var fid = res.files[0].fileid;
        try {
          _this.$request('get', '/ducar/dzgy/readxls_by_replace', {
            fileid: fid
          }).then(function (result) {
            _this.$loading().close();
            if (result.code === 1) {
    ...
    import_by_zh(_this, res) {
      if (res.files.length > 0) {
        var fid = res.files[0].fileid;
        try {
          _this.$request('get', '/ducar/dzgy/readxls_by_zh', {
            fileid: fid
          }).then(function (result) {
            _this.$loading().close();
            if (result.code === 1) {
  ...
  pagefuns: {
    ...
    download_template_file: function () {
      window.open('http://192.168.1.111:7002/template/Ducar/电子工艺.xlsx?r=' + Math.random());
    },
	view_dzgypdf:function(row){
		window.open("http://192.168.1.114:8100/储能/电子工艺/"+row.gymc);
	},
