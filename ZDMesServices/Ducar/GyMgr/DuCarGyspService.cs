...
namespace ZDMesServices.Ducar.GyMgr
{
    public class DuCarGyspService : BaseDao<zxjc_t_dzgy_sp>, IBatAtachValue<zxjc_t_dzgy_sp>
    {
        ...
        public override IEnumerable<zxjc_t_dzgy_sp> GetList(sys_page parm, out int resultcount)
        {
            try
            {
                ...
                sql_main.Append($"select ta.gyid, ta.gybh, ta.gymc, ta.gyms, ta.gcdm, ta.scx, ta.gwh, tb.gwmc, ta.jx_no as jxno, ta.status_no as statusno, ta.wjlj, ta.jwdx, ta.scry, ta.scpc, ta.scsj, ta.bbbh from zxjc_t_dzgy_sp ta, base_gwzd tb ");
                sql_main.Append(" where ta.gwh = tb.gwh(+) and ta.scx = tb.scx(+) ");
                sql.Append(sql_main);
                sql.Append(" ) zxjc_t_dzgy_sp where 1=1 ");
                sql_cnt.Append($"select count(*) from (");
                sql_cnt.Append(sql_main);
                sql_cnt.Append(" ) zxjc_t_dzgy_sp where 1=1 ");
                //
                ...
                using (var db = new OracleConnection(ConString))
                {
                    ...
                    var q = db.Query<zxjc_t_dzgy_sp>(OraPager(sql.ToString()), parm.sqlparam);
